...
def notifications(request , pk):
    ...
    return render(request, 'profile/notifications.html' , {'user_profile': user_profile ,'profile': profile, 'profile_object': profile_object, 'notifications' : notifications_count })
...
def inbox(request , pk):
    ...
    return render(request, 'profile/inbox.html' , {'user_profile': user_profile ,'profile': profile, 'profile_object': profile_object, 'notifications' : notifications_count })
...
def code_charge_function(request):
    ...
    if request.method == 'POST':
        ...
        if Code.objects.filter(code_id=code).first():
            ...
            student_profile .save()
            text = 'yes'
            new_notification = Notification.objects.create(user=student_user ,user_name=student_user.username , notification_type='charge'  )
