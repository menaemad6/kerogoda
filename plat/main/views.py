...
from .models import PlatformSettings, Profile , Lecture , StudentLectureObject, LectureCode ,  Part , StudentPartObject, View , Chapter , ChapterLecture , Group , GroupMember , GroupLecture , BuyLesson , BuyChapter
...
def index(request):
    ...
    return render(request, 'main/index.html', { 'lectures':lectures   , 'user_profile': user_profile ,  'notifications' : notifications_count , 'platform':platform})
...
def lecture_video(request , id , video):
    ...
    if Part.objects.filter(part_id=video).first():
        ...
        mode = 'video'
        part_views = View.objects.filter(part_id = part.part_id)
    else:
