...
def dashboard(request):
    ...
    # Comments
    comments = StudentQuestion.objects.filter(commented_to=request.user.username).order_by('created_at')
    ...
    questions = StudentQuestion.objects.filter(commented_to=request.user.username)
...
def add_part_to_lecture(request):
    ...
    if request.method == 'POST':
        ...
        copy_part = Part.objects.create(lecture_id=lecture_id , user=request.user , user_name=request.user.username , assignment_id=existing_part.assignment_id , type=type  , title=existing_part.title , video=existing_part.video , video_url=existing_part.video_url , youtube_url=existing_part.youtube_url , part_number=part_number , duration=existing_part.duration , views_limit=existing_part.views_limit , visible=existing_part.visible , link=existing_part.link , original=False)
        copy_part.save()
        ...
        for x in students:
            student_user = User.objects.get(username=x.username)
            create_part_object = StudentPartObject.objects.create(part_id=copy_part.part_id , lecture_id=copy_part.lecture_id , assignment_id=copy_part.assignment_id, user=student_user , user_name=student_user.username , type=copy_part.type , title=copy_part.title , part_number=copy_part.part_number , duration=copy_part.duration , views_limit=copy_part.views_limit , visible=copy_part.visible)
            create_part_object.save()
        ...
        redirction_link = str(lecture_id)
        messages.success(request, 'تم اضاقة الجزء الى المحاضرة بنجاح')
        return redirect('/lecture/' + redirction_link)
